...
```json
{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"any-harness","version":"1.0.0"}}}
...
When another harness launches `plugin.json`'s Python entry directly, install CodeGraph as a Python package or set `PYTHONPATH` to this plugin's `src` directory. The DSH Node bridge sets that path automatically.
...
└── tests/                    # 130 unit/integration tests (+ node:test bridge cases)
```
